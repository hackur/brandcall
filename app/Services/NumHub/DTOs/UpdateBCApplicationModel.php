...
/**
 * Update BC Application Model - Update Request.
 *
 * Used for PUT /api/v1/application/{NumhubEntityId} to update an existing application.
 */
final class UpdateBCApplicationModel extends BaseDTO
{
     ...
     * Note: Uses DisplayIdentity for updates (with numhubIdentityId).
     */
    ...
    public static function fromArray(array $data): static
    {
        ...
        if (isset($data['displayIdentityInformation']) && is_array($data['displayIdentityInformation'])) {
            $instance->displayIdentityInformation = DisplayIdentity::fromArray($data['displayIdentityInformation']);
        }
    ...
    /**
     * Track a modified section.
     *
     * @param  string  $sectionName  The section name
     * @return self
     */
    public function markSectionModified(string $sectionName): self
    {
        if ($this->modifiedSections === null) {
            $this->modifiedSections = [];
        }

        if (! in_array($sectionName, $this->modifiedSections, true)) {
            $this->modifiedSections[] = $sectionName;
        }
