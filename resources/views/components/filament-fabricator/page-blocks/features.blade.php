@props(['eyebrow', 'heading', 'description', 'features', 'columns'])

@php
    $columnClasses = match($columns ?? '4') {
        '2' => 'md:grid-cols-2',
        '3' => 'md:grid-cols-3',
    ...
    $icons = [
        ...
        'chart-trending-up' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />',
...
<section class="py-24 border-t border-slate-800/50">
    <div class="max-w-7xl mx-auto px-6">
        ...
        @if(!empty($features))
            <div class="grid {{ $columnClasses }} gap-6">
                @foreach($features as $feature)
                    <div class="p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50 hover:bg-slate-800/70 hover:border-slate-600/50 transition-colors duration-200">
                        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-600/10 text-brand-400 mb-4">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                {!! $icons[$feature['icon'] ?? 'check-circle'] ?? $icons['check-circle'] !!}
                            </svg>
                        </div>
